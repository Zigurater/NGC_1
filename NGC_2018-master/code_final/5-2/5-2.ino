...
if ((wormhole == false) and ((LswitchR == HIGH) or (LswitchL == HIGH)))  { //entering the wormhole
  delay(200); // This delay prevents the bot from thinking it is outside of the wormhole if the limit switches momentarily are unpressed.
...
else if ((wormhole == true) and (LswitchR == LOW) and (LswitchL == LOW) and (apds.readProximity() == 0)) { //leaving the wormhole
    wormhole = false;
